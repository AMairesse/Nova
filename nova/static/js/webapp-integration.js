...
(function () {
    ...
    window.WebappIntegration = {
        // Load webapps list into the desktop sidebar
        ...
        // Load webapps list into the mobile offcanvas container
        loadMobileWebappsList: async function () {
            const container = document.getElementById('webapps-list-container-mobile');
            if (!container) return;
            if (!window.FileManager.currentThreadId) {
                container.innerHTML = '<p class="text-muted p-3">No thread selected.</p>';
                return;
            ...
            try {
                ...
                console.error('Error loading mobile webapps list:', err);
                container.innerHTML = '<p class="text-danger p-3">Error loading webapps.</p>';
            }
        },
