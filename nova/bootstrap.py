...
def ensure_nova_agent(
    ...
    nova_prompt = (
        ...
        "is available. Keep ownership of thread files, cleanup, and live webapp publication in your own session. "
    ...
    detached_tool_agents.extend(
        list(
            nova_agent.agent_tools.filter(
                name__in=["Python Agent", "Code Agent"],
            )
        )
    )
...
def bootstrap_default_setup(user) -> Dict:
    ...
    if not main_provider:
        ...
        _skip_agents(summary, ["Internet Agent", "Nova", "Image Agent"], reason)
        logger.info(
