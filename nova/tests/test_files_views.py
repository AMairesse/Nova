...
from nova.runtime.constants import RUNTIME_STORAGE_ROOT
...
class FilesViewsTests(TestCase):
    ...
    @patch("nova.views.files_views.build_virtual_tree", return_value=[])
    def test_file_list_hides_runtime_tmp_files(self, mocked_tree):
        self._create_user_file(name="/visible.txt")
        UserFile.objects.create(
            user=self.user,
            thread=self.thread,
    ...
    @patch("nova.models.UserFile.UserFile.get_download_url", return_value="https://download.test/file-content")
    def test_file_content_redirects_to_signed_url(self, mocked_get_url):
        user_file = self._create_user_file(name="clip.mp3")

        response = self.client.get(reverse("file_content", args=[user_file.id]))

        self.assertEqual(response.status_code, 302)
        self.assertEqual(response["Location"], "https://download.test/file-content")
