...
class LLMAgentPromptTests(LLMAgentTestMixin, IsolatedAsyncioTestCase):
    ...
    async def test_build_system_prompt_default(self):
        ...
        with patch.object(llm_agent_mod.UserFile.objects, 'filter') as mock_filter:
            ...
            # Default helper prompt text
            self.assertIn("You are a helpful assistant", prompt)
            # Includes today's date text (not the literal {today} placeholder)
            self.assertNotIn("{today}", prompt)
            self.assertIn("Today is", prompt)
            # Includes explicit "no files" context by default
            self.assertIn("No attached files available.", prompt)
    ...
    async def test_build_system_prompt_with_file_context(self):
        """Test system prompt includes file attachment information when files exist."""
        ...
        with patch.object(llm_agent_mod.UserFile.objects, 'filter') as mock_filter:
            ...
            prompt = await agent.build_system_prompt()
            self.assertIn("3 file(s) are attached to this thread. Use file tools if needed.", prompt)
            # Should not contain the "no files" message
            self.assertNotIn("No attached files available.", prompt)

    async def test_build_system_prompt_no_files(self):
        """Test system prompt when no files are attached uses explicit message."""
        with patch.object(llm_agent_mod.UserFile.objects, 'filter') as mock_filter:
            mock_filter.return_value.count.return_value = 0
            ...
            prompt = await agent.build_system_prompt()
            self.assertIn("No attached files available.", prompt)
            self.assertNotIn("file(s) are attached to this thread. Use file tools if needed.", prompt)
